...
from gnumonitor.models import Data_Chart, Chart, Data_Error
        ...
        if request.method == "GET":
            ...
            if id_last_item == '0':
                ...
                for chart_object in charts_list:
                    obj_data_chart = Data_Chart.objects.filter(chart_object=chart_object).order_by('-pk')[:1]
                    if obj_data_chart:
                        last_id_to_get = obj_data_chart[0].pk
                        last_id_to_get = last_id_to_get - 10
                        data.extend(list(Data_Chart.objects.filter(id__gt=last_id_to_get, chart_object=chart_object).order_by("time").values()))
            else:
